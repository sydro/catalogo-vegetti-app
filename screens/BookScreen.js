...
class BookScreen extends React.Component {
  ...
  fetchBook = async () => {
    let bookUri = this.props.navigation.getParam('uri', '')
    if (bookUri !== '') {
      ...
      $('#main').each(function(i, item) {
        ...
        $(this)
          .find('.volume-info')
          .each(function(j, vinfo) {
            books[i]['info'].push($(this).text())
          })

        $(this)
          .find('#elenco-opere dt')
          .each(function(j, vinfo) {
            books[i]['opere'][j] = {
              titolo: $(this)
                .find('a')
  ...
  render() {
    let bookTitle = this.props.navigation.getParam('title', 'Nessun volume selezionato')

    let infos = null
    if (typeof this.state.books[0] !== 'undefined') {
      infos = this.state.books[0].info.map((e, index) => {
        return (
          <Text key={'info' + index} style={styles.info}>
            {e}
          </Text>
        )
      })
      ...
      opere = this.state.books[0].opere.map((e, index) => {
        return (
          <Row key={'row-opere' + index}>
            <Col size={20}>
              <Text key={'opere' + index} style={styles.subtitle}>
                {e.pagina}
              </Text>
            </Col>
            <Col size={80}>
              <Text key={'opere-other' + index} style={styles.subtitle}>
                {e.titolo} ({e.lunghezza}){' '}
                {e.author !== e.titolo ? <Text style={{ textDecorationLine: 'underline' }}>{e.author}</Text> : ''}
              </Text>
            </Col>
          </Row>
        )
